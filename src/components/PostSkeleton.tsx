...
const PostSkeleton: React.FC<PostSkeletonProps> = ({  id,
    ...
    // setGradientStartColor(Math.floor(Math.random() * 6) + 1);
    return (
        <Link to={`/gallery/${id}`}>
     ...
     <div className={`sm:w-[300px] bg-gradient-to-b from-tagColor-1 to-gray-800`} data-animated={animated} data-height={height} data-width={width} data-imageid = {imageId} data-objectFit = {objectFit}   Style={{--span:${postHeight}}}></div>
     <div className=" flex flex-col bg-gray-500 p-4 pt-2 rounded-b-sm desc mb-2">
     <p className="text-sm font-medium overflow-hidden mb-3 title sm:w-[260px]">{title}</p>
     <div className="flex justify-between text-gray-200 text-xs font-medium ">      
     <p className="flex gap-1 items-center justify-center"> <span className= "hover:text-btnColor-1"><svg height="16" width="16">
     ...
     <p className="flex gap-1 items-center justify-center hover:text-white"> <span><svg height="16" width="16">
